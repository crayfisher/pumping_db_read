...
lims1 <- c(parse_date_time("1/1/2022","dmy"),today())
...
pumping_db_proc_sumps3_filt <- pumping_db_proc_sumps3 %>%
  ...
  mutate(Qkm3 = Qkm3/7,
    date3  = floor_date(date,"week",week_start = wday(today()-2) ),
    date4 = floor_date(date,"month",week_start = wday(today()-2) )) %>% 
  mutate(gr7 = ifelse(gr4 == "bores deep" | gr4 ==  "bores shallow", gr4,gr6)) %>% 
  group_by(date3,gr1,gr2,gr7) %>%
...
write_excel_csv(pumping_db_proc_sumps3_filt,"pumping_db_proc_sumps3_filt.csv")
chart
...
str_replace("a a","\ ","b")
#bores vs trenches------------
ch_title <- "Brine Extraction from Bores and Trenches"

chart <- ggplot(pumping_db_proc_sumps3_filt,aes(date3,Qkm3,fill = gr2))+
  geom_bar(stat = "identity")+
  scale_fill_discrete(name = "Extraction group",
                       labels = c("Paleochannel Bores",
                                  "Shallow Bores",
                                  "Trenches - Sump 3",
                                  "Trenches - Sump 4",
                                  "Trenches - Sump 2"))+
...
bores_df2 <- bores_df %>%
  ...
  group_by(date3,pad,location,uid,gr2,shallow_gr) %>%
  summarise(Q= sum(Q))
# bores_df2 <- bores_df %>%
#   filter(date < today() -1) %>%
#   mutate(Q = Q/30,
...
fn_png <-   str_glue("{fd}pumping_{ch_title}_{tod}_{time_scale}d.png" )
fn_html <-   str_glue("{fd}pumping_{ch_title}_{tod}_{time_scale}d.html" )
ggsave(str_glue("{fd}pumping_{ch_title}_{tod}_{time_scale}d.png" ),width = 12, height =8)
ch_pl <- ggplotly(chart, dynamicTicks = T)
