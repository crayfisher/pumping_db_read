...
str_replace("a a","\ ","b")
#bores vs trenches------------
ch_title <- "bores vs trenches_long_term"
...
#   filter(date < today() -1) %>%
#   mutate(Q = Q/7,
#          date  = floor_date(date,"week",week_start = wday(today()-1-1) )) %>%
#   group_by(date,pad,location,gr2,shallow_gr) %>%
#   summarise(Q= sum(Q))
bores_df2 <- bores_df %>%
  filter(date < today() -1,
         date > today() - time_scale) 
...
#bores deep/shallow------------
ch_title <- "bores deep-shallow_long_term"
#colls----
...
chart <-  bores_df2 %>% 
  ...
  scale_x_datetime(date_breaks =  "2 months",
                   date_labels = "%b %y",
                   name = "Date",
                   limits = parse_date_time(c("1/6/2021","1/11/2021"),"dmy"))+
  ggtitle(ch_title)+
...
chart <-  bores_deep %>% 
  ...
  scale_x_datetime(date_breaks =  "3 months",
                   date_labels = "%b %y",
                   name = "Date",
                   limits = parse_date_time(c("1//6/2024","1/11/2021"),"dmy"))+
  ggtitle(ch_title)+
  ylab("Brine Extraction 1000 m3")
...
chart <-  filter(bores_shallow_high_Q,pad %in% c("Pad 28","Pad 14","Pad 29") )%>% 
  ...
  scale_x_datetime(date_breaks =  "3 months",
                   date_labels = "%b %y",
                   name = "Date",
                   limits = parse_date_time(c("1/1/2020","1/11/2021"),"dmy"))+
  ggtitle(ch_title)+
  ylab("Brine Extraction 1000 m3")
...
chart <-  filter(bores_shallow_low_Q,pad %in% c("Pad 18","Pad 17","Pad 21", "Pad 23") ) %>% 
  ggplot(aes(date2,Q, fill = pad))+
  geom_bar(stat = "identity")+
  facet_wrap(~location,
             ncol =1,
...
# bores deep --------
#ch_title <- "bores deep_mth"
ch_title <- "Long Term Aquifer Pumping"



lims1 <- parse_date_time(c("1/11/2020","1/11/2021"),"dmy")

bores_deep_mth <- bores_df2 %>% 
  filter(location %in% c("Pad 08", "Pad 14s", "Pad 17" )) %>%
  # filter(date >= lims1[1],date >= lims[2]) %>%
  
  group_by(location,date2) %>%
  summarise(Q=sum(Q)) %>% 
  mutate(location = factor(location,levels = c("Pad 17","Pad 08","Pad 14s")))
  
write_excel_csv(bores_deep_mth,"bores_deep_mth.csv")
chart <-  bores_deep_mth %>% 
    
  ggplot(aes(date2,Q,fill = location))+
  geom_bar(stat = "identity",
           position = "stack")+
   facet_wrap(~location,
              ncol = 1,
  ...
  scale_x_datetime(date_breaks =  "2 months",
                   date_labels = "%b %y",
                   name = "Date",
                   limits = parse_date_time(c("1/7/2020","1/11/2021"),"dmy"))+
  #ggtitle(ch_title)+

  ylab(expression(paste("Brine Extraction 1000   ", m^3, "/day")))+
...
fn_png <-   str_glue("{fd}pumping_{ch_title}_{tod}_{time_scale}d.png" )
fn_html <-   str_glue("{fd}pumping_{ch_title}_{tod}_{time_scale}d.html" )
ggsave(str_glue("{fd}pumping_{ch_title}_{tod}_{time_scale}d.png" ),width = 8, height = 4.5)
ch_pl <- ggplotly(chart, dynamicTicks = T)
