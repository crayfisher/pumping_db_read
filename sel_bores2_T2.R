...
bores_df_sel <- bores_df %>% 
  ...
  mutate(ID3 = as.factor(str_glue("{uid}_{dist}km "))) %>% 
  filter(dist <=7)

bores_df_sel
bores_df_sel %>% distinct(uid,dist) %>% arrange(dist)
ref_df
ch_title <- "Bore Pumping"
chart <-  bores_df_sel %>% 
...
fn_png <-   str_glue("{fd}{ch_title}_{tod}_{time_scale}d.png" )
ggsave(fn_png,width = 7, height = 10)
#trenches#####
...
bore_sel_pat_hydro_uid <- "PD016M2|^T2"
bore_sel_pat_hydro_hid <- "Ur|LV_A|LYAGZ007|LYAGZ026"
