...
ref_list_sumps <- read_csv("ref_sump_db_read.csv") %>% 
  ...
  pivot_longer(-date,
               names_to = "location",
               values_to = "sump_gr" ) %>% 
  mutate(sump_gr = if_else(sump_gr != "transfer","trenches","transfer" ))
...
pumping_db_proc <- pumping_db %>% 
  ...
  mutate(gr2 = ifelse(gr1 == "trenches",gr1,gr2 )) %>% 
  dplyr::select(-sump_gr) %>% 
  filter(gr2 != "transfer") %>% 
  mutate(date2= floor_date(date,unit = "week",week_start = 6))

#write_excel_csv(pumping_db_proc, "pumping_db_proc.csv")
...
pumping_db_proc_corr_trenches <- pumping_db_proc %>% 
  filter( gr1 == "trenches") %>% 
...
tail(pumping_db_proc_corr_trenches)
